...
pkill_agent_cmdline() {
    ...
    [[ -n "$path" ]] || return 0
    pattern="^$(regex_escape_ere "$path")( |$)"
    ${SUDO} pkill --signal "$signal_name" -f "$pattern" 2>/dev/null || true
}
...
pkill_agent_cmdline TERM "${BINARY_DST}"
pkill_agent_cmdline TERM "${BINARY_SRC}"
sleep 1
pkill_agent_cmdline KILL "${BINARY_DST}"
pkill_agent_cmdline KILL "${BINARY_SRC}"
