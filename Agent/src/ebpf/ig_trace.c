...
static void path_cache_put(__u64 dev, __u64 ino, const char *path)
{
    ...
    for (uint32_t i = 0; i < PATH_CACHE_CAP; i++) {
        ...
        if (e->used == SLOT_EMPTY) {
            ...
            t->ino  = ino;
            t->used = SLOT_USED;
            strncpy(t->path, path, IG_MAX_PATH - 1);
            t->path[IG_MAX_PATH - 1] = '\0';
            break;
        }
...
{
    switch (mask) {
        case IG_EBPF_OP_READ:   return "READ";
        case IG_EBPF_OP_WRITE:  return "WRITE";
        case IG_EBPF_OP_DELETE: return "DELETE";
        case IG_EBPF_OP_ATTR:   return "ATTR";
...
static int add_path_internal(const char *path, __u32 mask, __u32 block, int recursive)
{
    ...
    if (lstat(path, &st) < 0) {
        LOG_WARN_IG("[ebpf] stat failed: %s (%s)", path, strerror(errno));
        return -1;
    }
    ...
    if (add_policy_inode(kdev, kino, mask, block) == 0) {
        ...
        LOG_INFO_IG("[ebpf] policy add: %s dev=%llu ino=%llu mask=0x%x block=%u",
                     path, (unsigned long long)kdev, (unsigned long long)kino,
                     mask, block);
    } else {
        LOG_WARN_IG("[ebpf] policy add failed: %s (%s)", path, strerror(errno));
    }
    ...
    DIR *dir = opendir(path);
    if (!dir) {
        LOG_WARN_IG("[ebpf] opendir failed: %s (%s)", path, strerror(errno));
        return -1;
    }
...
static ig_event_type_t op_to_ig_type(__u32 op_mask)
{
    if (op_mask & IG_EBPF_OP_DELETE) return IG_EVENT_DELETE;
    if (op_mask & IG_EBPF_OP_WRITE)  return IG_EVENT_MODIFY;
    if (op_mask & IG_EBPF_OP_ATTR)   return IG_EVENT_ATTRIB;
...
static int handle_audit_event(void *ctx, void *data, size_t data_sz)
{
    ...
    if (e->hook_id == IG_HOOK_PATH_UNLINK && !(e->denied)) {
        remove_policy_inode(e->dev, e->ino);
        path_cache_remove(e->dev, e->ino);
    }

    /* 큐가 있으면 ig_event_t로 변환해서 push → 무결성 검사 + transport 전송 */
    if (queue) {
        ig_event_t ev;
...
int ebpf_policy_init(ig_event_queue_t *queue)
{
    int err;

    g_skel = ig_trace_bpf__open();
    if (!g_skel) {
        fprintf(stderr, "eBPF open failed\n");
        return -1;
    }

    err = ig_trace_bpf__load(g_skel);
    if (err) {
        fprintf(stderr, "eBPF load failed: %d\n", err);
        ig_trace_bpf__destroy(g_skel);
        g_skel = NULL;
        return -1;
    }

    err = ig_trace_bpf__attach(g_skel);
    if (err) {
    ...
    /* queue를 ctx로 전달 → handle_audit_event에서 ig_queue_push 사용 */
