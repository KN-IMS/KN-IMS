 ...
 * 우선순위:
 *   eBPF 가능 → eBPF
 *   eBPF 불가 + /dev/ig_lkm 존재 → LKM
 *   둘 다 불가 → 시작 중단
 *
 * 환경변수 (transport 설정):
 *   IG_SERVER_HOST   서버 IP/호스트 (기본: 127.0.0.1)
 *   IG_SERVER_PORT   서버 포트      (기본: 9000)
 *   IG_CA_CRT        CA 인증서 경로  (기본: /etc/ig_monitor/certs/ca.crt)
 *   IG_AGENT_CRT     에이전트 인증서 (기본: /etc/ig_monitor/certs/agent.crt)
 *   IG_AGENT_KEY     에이전트 개인키 (기본: /etc/ig_monitor/certs/agent.key)
 *
 * 시그널:
...
static int current_transport_monitor_type(uint8_t *monitor_type)
{
    ...
    if (g_ebpf_active) {
        *monitor_type = IG_MON_EBPF;
        return 0;
    }
    if (g_lkm_active) {
        *monitor_type = IG_MON_LKM;
        return 0;
    }
    return -1;
}
...
/* ── 로깅 ──────────────────────────────────────── */
static int init_logging(ig_config_t *cfg) {
    if (cfg->log_to_syslog) {
        openlog("ig_monitor", LOG_PID | LOG_NDELAY, LOG_DAEMON);
        g_use_syslog = 1;
    }
...
{
    ig_event_queue_t    *queue = (ig_event_queue_t *)arg;
    struct ig_lkm_event  lev;

    LOG_INFO_IG("[lkm] 이벤트 수신 스레드 시작 (tid=%ld)", (long)pthread_self());
    while (g_running) {
        ...
        ig_event_t ev = {0};
        switch (lev.op) {
        case IG_OP_WRITE:  ev.type = IG_EVENT_MODIFY; break;
        case IG_OP_DELETE: ev.type = IG_EVENT_DELETE; break;
        case IG_OP_RENAME: ev.type = IG_EVENT_MOVE;   break;
        default:            ev.type = IG_EVENT_UNKNOWN; break;
        }
...
static void process_event(const ig_event_t *ev) {
    ...
    if (ev->source != IG_SOURCE_EBPF && ev->source != IG_SOURCE_LKM) {
        if (ev->pid > 0) {
            LOG_ALERT_IG("[%s] %s %s  (pid=%d uid=%d sid=%d comm=%s)",
                          ig_source_str(ev->source),
                          ev->path, ig_event_type_str(ev->type),
    ...
    /* 베이스라인 갱신 및 무결성 검사 */
    switch (ev->type) {
    case IG_EVENT_MODIFY: {
        char expected[65] = {0}, actual[65] = {0};
        ig_integrity_result_t r = ig_baseline_check_file(&g_baseline_db,
                                                           ev->path,
                                                           expected, actual);
        if (r == IG_INTEGRITY_MISMATCH) {
            LOG_ALERT_IG("[integrity] 변조 감지: %s  expected=%s  actual=%s",
                          ev->path, expected, actual);
        } else if (r == IG_INTEGRITY_NEW) {
            LOG_INFO_IG("[baseline] 베이스라인 미등록 파일 수정 감지 — 신규 등록: %s",
                         ev->path);
        }
        /* 변조 여부와 무관하게 현재 상태를 베이스라인에 반영 */
        ig_baseline_db_update(&g_baseline_db, ev->path);
        break;
    }
    case IG_EVENT_CREATE:
        ig_baseline_db_update(&g_baseline_db, ev->path);
        LOG_INFO_IG("[baseline] 신규 파일 등록: %s", ev->path);
        break;
    case IG_EVENT_DELETE: {
        char expected[65] = {0}, actual[65] = {0};
        ig_integrity_result_t r = ig_baseline_check_file(&g_baseline_db,
                                                            ev->path,
                                                            expected, actual);
        /* DENY로 차단된 삭제: 파일이 실제로 존재 → MATCH → 베이스라인 유지 */
        if (r == IG_INTEGRITY_MATCH) break;
        ig_baseline_db_remove(&g_baseline_db, ev->path);
        LOG_INFO_IG("[baseline] 파일 삭제 — 베이스라인 제거: %s", ev->path);
        break;
    }
    default:
        break;
...
static void reload_config(void) {
    ig_config_t *new_cfg = calloc(1, sizeof(ig_config_t));
    if (!new_cfg) {
        LOG_ERROR_IG("configuration reload failed: Out of Memory");
        return;
    }

    if (ig_config_load(new_cfg, g_config_path) < 0) {
        LOG_ERROR_IG("configuration reload failed: %s", g_config_path);
        free(new_cfg);
        return;
    }

    LOG_INFO_IG("SIGHUP — configuration reload start: %s", g_config_path);
    ...
    if (strcmp(g_cfg->log_file, new_cfg->log_file) != 0 ||
        ...
        init_logging(g_cfg);
        LOG_INFO_IG("log file config changes applied");
    ...
    if (g_ebpf_active) {
        ...
        for (int i = 0; i < g_cfg->watch_count; i++) {
            ...
            if (!found) {
                ...
                LOG_INFO_IG("[ebpf] policy removed: %s", g_cfg->watches[i].path);
            }
        }
        ...
        for (int j = 0; j < new_cfg->watch_count; j++) {
            ...
            if (!found) {
                ...
                LOG_INFO_IG("[ebpf] policy added: %s", new_cfg->watches[j].path);
            }
        }
    }
#endif /* HAVE_LIBBPF */
...
/* 탐색 경로: -c/-e 플래그 > /etc/ig_monitor/ 만 허용 (CWD·HOME 제거) */
...
static void build_cert_path(char *buf, size_t len, const char *filename) {
    /* 인증서 기본 경로: /etc/ig_monitor/certs/ */
    snprintf(buf, len, "/etc/ig_monitor/certs/%s", filename);
}
...
static void print_usage(const char *prog) {
    ...
    printf("\tIG_SERVER_HOST\t서버 IP (기본: 127.0.0.1)\n");
    printf("\tIG_SERVER_PORT\t서버 포트 (기본: 9000)\n");
...
int main(int argc, char *argv[]) {
        ...
        switch (opt) {
            ...
            case 'm':
                if (strcmp(optarg, "maintenance") == 0)
                    g_ebpf_block = IG_EBPF_BLOCK_AUDIT;
                else if (strcmp(optarg, "lock") == 0)
                    g_ebpf_block = IG_EBPF_BLOCK_DENY;
                else {
    ...
    /* conf 탐색: -c 플래그 > /etc/ig_monitor/ig.conf (CWD·HOME 제거) */
    ...
    const char *env_path = find_system_file(flag_env, "/etc/ig_monitor/ig.env");
    ...
    if (ig_config_load(cfg, g_config_path) < 0) {
    ...
    if (!cfg->daemonize && strcmp(cfg->log_file, IG_LOG_FILE) == 0)
    ...
    LOG_INFO_IG("╔══════════════════════════════════════╗");
    LOG_INFO_IG("║   KN-IG                              ║");
    LOG_INFO_IG("║   pid=%d  kernel=%d.%d  mode=%s",
    ...
    if (cfg->watch_count == 0) {
        LOG_ERROR_IG("target does not exist ([watch] section check) — exit");
        close_logging();
        free(cfg);
        return 1;
    }
    ...
    if (cfg->daemonize) {
        close_logging();
        init_logging(cfg);
        LOG_INFO_IG("daemon PID=%d", getpid());
    }

    if (pid_lock_acquire(IG_PID_FILE) < 0) {
        LOG_ERROR_IG("Already running or PID lock failed");
        close_logging();
        free(cfg);
        return 1;
    }
    ...
    /* ── 이벤트 큐 초기화 ─────────────────────── */
    ig_event_queue_t *queue = calloc(1, sizeof(ig_event_queue_t));
    if (!queue || ig_queue_init(queue) < 0) {
        LOG_ERROR_IG("event queue initialize failed");
        free(queue);
        goto done;
    }
    ...
    if (cfg->ebpf_enabled && kver >= KERNEL_VER(5, 8)) {
        LOG_INFO_IG("eBPF inode policy backend activate (kernel %d.%d)",
                     kver_major, kver_minor);
        if (ebpf_policy_init(queue) < 0) {
            LOG_WARN_IG("eBPF init failed — inode policy disabled");
        } else {
            g_ebpf_active = 1;
            pthread_create(&ebpf_thread, NULL, ebpf_poll_thread, NULL);
        }
    } else {
        LOG_INFO_IG("eBPF Disabled (kernel %d.%d, required: 5.8+)",
                     kver_major, kver_minor);
    }
#else
    {
    ...
    if (!g_ebpf_active) {
        /* LKM 가용 여부 확인 — /dev/ig_lkm 존재 시 활성화 */
        if (access(IG_LKM_DEV_PATH, F_OK) == 0) {
            if (lkm_client_init() == 0) {
                g_lkm_active = 1;
                LOG_INFO_IG("[lkm] ig_lkm.ko 감지 — LKM 모드 활성 (mode=%s)",
                             g_ebpf_block ? "lock(DENY)" : "maintenance(AUDIT)");
            } else {
                LOG_WARN_IG("[lkm] %s 열기 실패 — LKM 비활성", IG_LKM_DEV_PATH);
            }
        } else {
            LOG_INFO_IG("[lkm] %s 없음 — LKM 비활성", IG_LKM_DEV_PATH);
        }
    }
    ...
    const char *server_host = getenv("IG_SERVER_HOST");
    ...
    if (ig_baseline_db_init(&g_baseline_db) < 0) {
        LOG_ERROR_IG("[baseline] DB 초기화 실패");
        goto done;
    }
    LOG_INFO_IG("[baseline] 초기 스캔 시작 — 모든 감시 경로 해시 수집...");
    int baseline_count = ig_baseline_db_build(&g_baseline_db, cfg);
    if (baseline_count < 0)
        LOG_WARN_IG("[baseline] 초기 스캔 실패 — 무결성 검사 불완전할 수 있음");
    else
        LOG_INFO_IG("[baseline] 초기 스캔 완료 — %d개 파일 등록", baseline_count);
    ...
    if (g_lkm_active) {
        int n = lkm_add_from_baseline(&g_baseline_db,
                                      g_ebpf_block ? IG_BLOCK_DENY
                                                    : IG_BLOCK_AUDIT);
        if (n < 0)
            LOG_WARN_IG("[lkm] 정책 주입 실패");
        else
            LOG_INFO_IG("[lkm] %d개 inode 정책 등록 완료", n);

        if (pthread_create(&lkm_thread, NULL, lkm_event_thread, queue) != 0) {
            LOG_WARN_IG("[lkm] 이벤트 스레드 생성 실패");
        }
    }
    ...
    if (g_ebpf_active) {
        uint32_t protect_mask = IG_EBPF_OP_WRITE | IG_EBPF_OP_DELETE | IG_EBPF_OP_ATTR;
        LOG_INFO_IG("[ebpf] mode=%s — 보호 경로 %d개 등록 중...",
                     g_ebpf_block ? "lock (DENY)" : "maintenance (AUDIT)",
                     cfg->watch_count);
        for (int i = 0; i < cfg->watch_count; i++) {
            if (ebpf_policy_add_path_recursive(cfg->watches[i].path,
                                               protect_mask,
                                               g_ebpf_block) < 0) {
                LOG_WARN_IG("[ebpf] policy 등록 실패: %s", cfg->watches[i].path);
            }
        }
    }
#endif /* HAVE_LIBBPF */
    ...
    LOG_INFO_IG("event process start..");
    ...
    while (g_running) {
        ...
        ig_event_t ev;
        if (ig_queue_pop(queue, &ev, 500) == 0)
            process_event(&ev);

        /* 이벤트 드롭 감지 — 큐 오버플로우 발생 시 경고 */
        uint64_t cur_dropped = ig_queue_dropped(queue);
        if (cur_dropped != last_dropped) {
            LOG_WARN_IG("event queue overflow: dropped %llu (+%llu)",
    ...
    if (ebpf_thread)  { pthread_join(ebpf_thread,  NULL); LOG_INFO_IG("eBPF 스레드 합류"); }
    ...
    LOG_INFO_IG("IG Monitor stopped");
    close_logging();
    free(cfg);
    return 0;
}
