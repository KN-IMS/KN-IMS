 ...
 *   SIGHUP          → 설정 재로드
 *   SIGUSR1         → /tmp/fim_scan_request 파일에서 경로 읽어 watch 추가/제거
 */
...
static fim_baseline_db_t g_baseline_db;

/* 백엔드를 전역으로 — handle_scan_request(), reload_config()에서 접근 필요 */
static fim_backend_t *g_be_inotify  = NULL;
static int            g_ebpf_active = 0;
static int            g_lkm_active  = 0;
static uint32_t       g_ebpf_block  = FIM_EBPF_BLOCK_DENY;

/* 설정 재로드를 위한 전역 — SIGHUP 핸들러에서 접근 */
static fim_config_t  *g_cfg         = NULL;
static char           g_config_path[FIM_MAX_PATH];

/* ── transport 전역 상태 ───────────────────────── */
static fim_tls_ctx_t    g_tls_ctx;
static fim_tcp_client_t g_tcp_client;
...
/* ── 로깅 ──────────────────────────────────────── */
static int init_logging(fim_config_t *cfg) {
    if (cfg->log_to_syslog) {
        openlog("fim_monitor", LOG_PID | LOG_NDELAY, LOG_DAEMON);
        g_use_syslog = 1;
    }
...
{
    fim_event_queue_t    *queue = (fim_event_queue_t *)arg;
    struct fim_lkm_event  lev;
    ...
    while (g_running) {
        ...
        fim_event_t ev = {0};
        switch (lev.op) {
        case FIM_OP_WRITE:  ev.type = FIM_EVENT_MODIFY; break;
        case FIM_OP_DELETE: ev.type = FIM_EVENT_DELETE; break;
        case FIM_OP_RENAME: ev.type = FIM_EVENT_MOVE;   break;
        default:            ev.type = FIM_EVENT_UNKNOWN; break;
        }
        ev.source    = FIM_SOURCE_LKM;
...
static void process_event(const fim_event_t *ev) {
    /* eBPF·LKM 이벤트는 각 수신 스레드에서 이미 로그 출력 완료 → 중복 스킵 */
    if (ev->source != FIM_SOURCE_EBPF && ev->source != FIM_SOURCE_LKM) {
        if (ev->pid > 0) {
            LOG_ALERT_FIM("[%s] %s %s  (pid=%d uid=%d sid=%d comm=%s)",
                          fim_source_str(ev->source),
                          ev->path, fim_event_type_str(ev->type),
    ...
    /* 베이스라인 갱신 및 무결성 검사 */
    switch (ev->type) {
    case FIM_EVENT_MODIFY: {
        char expected[65] = {0}, actual[65] = {0};
        fim_integrity_result_t r = fim_baseline_check_file(&g_baseline_db,
                                                           ev->path,
                                                           expected, actual);
        ...
        fim_baseline_db_update(&g_baseline_db, ev->path);
        break;
    }
    case FIM_EVENT_CREATE:
        fim_baseline_db_update(&g_baseline_db, ev->path);
        LOG_INFO_FIM("[baseline] 신규 파일 등록: %s", ev->path);
        break;
    case FIM_EVENT_DELETE: {
        char expected[65] = {0}, actual[65] = {0};
        fim_integrity_result_t r = fim_baseline_check_file(&g_baseline_db,
                                                            ev->path,
                                                            expected, actual);
        /* DENY로 차단된 삭제: 파일이 실제로 존재 → MATCH → 베이스라인 유지 */
        if (r == FIM_INTEGRITY_MATCH) break;
        fim_baseline_db_remove(&g_baseline_db, ev->path);
...
static void reload_config(void) {
    fim_config_t *new_cfg = calloc(1, sizeof(fim_config_t));
    if (!new_cfg) {
        LOG_ERROR_FIM("configuration reload failed: Out of Memory");
        return;
    }

    if (fim_config_load(new_cfg, g_config_path) < 0) {
    ...
    if (g_be_inotify) {
        ...
        memcpy(g_cfg->watches, new_cfg->watches,
               sizeof(fim_watch_entry_t) * (size_t)new_cfg->watch_count);
    }

    /* eBPF policy diff — inotify와 동일한 기준으로 policy_map 갱신 */
#ifdef HAVE_LIBBPF
    if (g_ebpf_active) {
        uint32_t protect_mask = FIM_EBPF_OP_WRITE | FIM_EBPF_OP_DELETE | FIM_EBPF_OP_ATTR;
...
#define FIM_SCAN_REQUEST_FILE "/tmp/fim_scan_request"
...
static void handle_scan_request(void) {
    ...
    FILE *fp = fopen(FIM_SCAN_REQUEST_FILE, "r");
    if (!fp) {
        LOG_WARN_FIM("scan request fle does not exist: %s", FIM_SCAN_REQUEST_FILE);
        return;
    }
    char line[FIM_MAX_PATH];
...
int main(int argc, char *argv[]) {
    ...
    int opt_verbose    = 0;
    g_ebpf_block = FIM_EBPF_BLOCK_DENY;  /* 기본: lock 모드 */
        ...
        switch (opt) {
            ...
            case 'm':
                if (strcmp(optarg, "maintenance") == 0)
                    g_ebpf_block = FIM_EBPF_BLOCK_AUDIT;
                else if (strcmp(optarg, "lock") == 0)
                    g_ebpf_block = FIM_EBPF_BLOCK_DENY;
                else {
    ...
    fim_config_t *cfg = calloc(1, sizeof(fim_config_t));
    if (!cfg) { fprintf(stderr, "Out of Memory\n"); return 1; }
    g_cfg = cfg;

    if (fim_config_load(cfg, g_config_path) < 0) {
    ...
    if (!cfg->daemonize && strcmp(cfg->log_file, FIM_LOG_FILE) == 0)
    ...
    if (pid_lock_acquire(FIM_PID_FILE) < 0) {
    ...
    /* ── 이벤트 큐 초기화 ─────────────────────── */
    fim_event_queue_t *queue = calloc(1, sizeof(fim_event_queue_t));
    if (!queue || fim_queue_init(queue) < 0) {
...
#else
    {
        ...
        /* LKM 가용 여부 확인 — /dev/fim_lkm 존재 시 활성화 */
        if (access(FIM_LKM_DEV_PATH, F_OK) == 0) {
            if (lkm_client_init() == 0) {
                g_lkm_active = 1;
                LOG_INFO_FIM("[lkm] fim_lkm.ko 감지 — LKM 모드 활성 (mode=%s)",
                             g_ebpf_block ? "lock(DENY)" : "maintenance(AUDIT)");
            } else {
                LOG_WARN_FIM("[lkm] %s 열기 실패 — LKM 비활성", FIM_LKM_DEV_PATH);
            }
        }
    }
    ...
    /* ── 로컬 베이스라인 구축 ─────────────────── */
    if (fim_baseline_db_init(&g_baseline_db) < 0) {
        LOG_ERROR_FIM("[baseline] DB 초기화 실패");
        goto done;
    }
    LOG_INFO_FIM("[baseline] 초기 스캔 시작 — 모든 감시 경로 해시 수집...");
    int baseline_count = fim_baseline_db_build(&g_baseline_db, cfg);
    if (baseline_count < 0)
    ...
    if (g_ebpf_active) {
        ...
        g_be_inotify = fim_inotify_create();
    ...
    while (g_running) {
        ...
        fim_event_t ev;
        if (fim_queue_pop(queue, &ev, 500) == 0)
            process_event(&ev);

        /* 이벤트 드롭 감지 — 큐 오버플로우 발생 시 경고 */
        uint64_t cur_dropped = fim_queue_dropped(queue);
