#ifndef IM_BASELINE_H
#define IM_BASELINE_H
...
typedef struct {
    im_scan_result_t data;
    pthread_rwlock_t  lock;
} im_baseline_db_t;

/* ── 무결성 검사 결과 ─────────────────────────────── */
typedef enum {
    IM_INTEGRITY_MATCH    =  0,  /* 해시 일치 */
    IM_INTEGRITY_MISMATCH =  1,  /* 해시 불일치 (변조 의심) */
    IM_INTEGRITY_NEW      =  2,  /* 베이스라인에 없는 신규 파일 */
    IM_INTEGRITY_ERROR    = -1,  /* 해시 계산 실패 */
} im_integrity_result_t;

/*
 * im_sha256_file — 파일 SHA-256 계산
...
/*
 * im_baseline_scan — cfg->watches 경로 전체 순회 후 결과 수집
 *   반환값 : 스캔한 파일 수, -1 오류
 */
int im_baseline_scan(im_config_t *cfg, im_scan_result_t *out);

/* im_scan_result_free — 동적 할당 해제 */
...
/*
 * im_baseline_check_file — MODIFY 이벤트 시 무결성 검사
 *   path         : 검사할 파일 경로
 *   out_expected : 베이스라인 해시 (65바이트 버퍼, 없으면 "" 반환)
 *   out_actual   : 현재 파일 해시 (65바이트 버퍼)
 *   반환값       : im_integrity_result_t
 */
