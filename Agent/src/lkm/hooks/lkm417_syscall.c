...
static long ig_check(uint64_t dev, uint64_t ino, uint32_t op,
                      const char *op_name)
{
    uint32_t block_mode;
...
static long ig_check_delete(uint64_t dev, uint64_t ino)
{
    uint32_t block_mode;
                         ...
                         int newdfd, const char __user *newname)
{
    ...
    if (pathat_to_devino(olddfd, oldname, &dev, &ino)) {
        rc = ig_check(dev, ino, IG_OP_RENAME, "rename(src)");
        if (rc)
            return rc;
    }

    if (pathat_to_devino(newdfd, newname, &dev, &ino)) {
        if (inode_policy_lookup(dev, ino, IG_OP_RENAME, &block_mode)
            && block_mode == IG_BLOCK_DENY) {
            pr_info("DENY rename(dst): comm=%s dev=%llu ino=%llu\n",
                    current->comm, dev, ino);
            ig_event_enqueue(dev, ino, IG_OP_RENAME, 1);
            return -EPERM;
        }
    }
...
static long check_open_flags(int dfd, const char __user *upath, int flags)
{
    ...
    return ig_check(dev, ino, IG_OP_WRITE, "open");
}

static asmlinkage long ig_sys_write(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (fd_to_devino((unsigned int)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "write");

    return rc ? rc : orig_sys_write(regs);
}

static asmlinkage long ig_sys_truncate(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (path_to_devino((const char __user *)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "truncate");

    return rc ? rc : orig_sys_truncate(regs);
}

static asmlinkage long ig_sys_ftruncate(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (fd_to_devino((unsigned int)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "ftruncate");

    return rc ? rc : orig_sys_ftruncate(regs);
}

static asmlinkage long ig_sys_open(const struct pt_regs *regs)
{
...
static asmlinkage long ig_sys_openat(const struct pt_regs *regs)
{
...
static asmlinkage long ig_sys_creat(const struct pt_regs *regs)
{
    long rc = check_open_flags(AT_FDCWD,
                               (const char __user *)arg1(regs),
                               O_WRONLY | O_TRUNC);
...
static asmlinkage long ig_sys_unlink(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (path_to_devino((const char __user *)arg1(regs), &dev, &ino))
        rc = ig_check_delete(dev, ino);

    return rc ? rc : orig_sys_unlink(regs);
}

static asmlinkage long ig_sys_unlinkat(const struct pt_regs *regs)
{
...
static asmlinkage long ig_sys_rename(const struct pt_regs *regs)
{
...
static asmlinkage long ig_sys_renameat(const struct pt_regs *regs)
{
...
#ifdef __NR_renameat2
static asmlinkage long ig_sys_renameat2(const struct pt_regs *regs)
{
    long rc = check_rename((int)arg1(regs),
                           (const char __user *)arg2(regs),
                           (int)arg3(regs),
...
static asmlinkage long ig_sys_chmod(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (path_to_devino((const char __user *)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "chmod");

    return rc ? rc : orig_sys_chmod(regs);
}

static asmlinkage long ig_sys_fchmodat(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;
    ...
    if (pathat_to_devino((int)arg1(regs),
        ...
        rc = ig_check(dev, ino, IG_OP_WRITE, "fchmodat");

    return rc ? rc : orig_sys_fchmodat(regs);
}

static asmlinkage long ig_sys_chown(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (path_to_devino((const char __user *)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "chown");

    return rc ? rc : orig_sys_chown(regs);
}

static asmlinkage long ig_sys_fchownat(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;
    ...
    if (pathat_to_devino((int)arg1(regs),
                         (const char __user *)arg2(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "fchownat");

    return rc ? rc : orig_sys_fchownat(regs);
}

static asmlinkage long ig_sys_link(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (path_to_devino((const char __user *)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "link");

    return rc ? rc : orig_sys_link(regs);
}

static asmlinkage long ig_sys_linkat(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;
    ...
    if (pathat_to_devino((int)arg1(regs),
                         (const char __user *)arg2(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "linkat");

    return rc ? rc : orig_sys_linkat(regs);
}

static asmlinkage long ig_sys_setxattr(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (path_to_devino((const char __user *)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "setxattr");

    return rc ? rc : orig_sys_setxattr(regs);
}

static asmlinkage long ig_sys_lsetxattr(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (path_to_devino((const char __user *)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "lsetxattr");

    return rc ? rc : orig_sys_lsetxattr(regs);
}

static asmlinkage long ig_sys_fsetxattr(const struct pt_regs *regs)
{
    uint64_t dev, ino;
    long rc = 0;

    if (fd_to_devino((int)arg1(regs), &dev, &ino))
        rc = ig_check(dev, ino, IG_OP_WRITE, "fsetxattr");
...
#define HOOK(name) \
    ...
    sys_call_table[__NR_##name] = ig_sys_##name
