...
/*
 * lkm_client.h — fim_monitor 유저스페이스 ↔ fim_lkm.ko 통신 인터페이스
 *
 * 사용 순서:
 *   1. lkm_client_init()           — /dev/fim_lkm 열기
 ...
 *   db    : fim_baseline_db_build() 완료된 DB
 *   block : FIM_BLOCK_DENY(차단) or FIM_BLOCK_AUDIT(탐지만)
 *   반환  : 등록된 inode 수, 음수는 오류
 */
int  lkm_add_from_baseline(fim_baseline_db_t *db, uint32_t block);

/* ── 이벤트 수신 ────────────────────────────────── */
int  lkm_read_event(struct fim_lkm_event *ev);           /* 무한 대기 */
int  lkm_read_event_timeout(struct fim_lkm_event *ev,
                             int timeout_ms);             /* 타임아웃 */
