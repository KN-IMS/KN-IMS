...
/*
 * ig_lkm_policy.c — inode based policy hash table
 *
...
DEFINE_HASHTABLE(ig_policy_ht, IG_POLICY_BITS);
DEFINE_RWLOCK(ig_policy_lock);

void ig_policy_init(void)
{
    hash_init(ig_policy_ht);
}
...
static u32 make_key(uint64_t dev, uint64_t ino)
{
    ...
    return jhash2(data, 4, 0) & ((1U << IG_POLICY_BITS) - 1);
}

/* Search for entry without lock — must be called while holding read/write_lock */
static struct ig_policy_entry *find_entry(uint64_t dev, uint64_t ino)
{
    struct ig_policy_entry *e;
    u32 key = make_key(dev, ino);

    hash_for_each_possible(ig_policy_ht, e, node, key) {
                     ...
                     const char *path)
{
    ...
    write_lock_irqsave(&ig_policy_lock, flags);
    e = find_entry(dev, ino);
    if (e) {
        e->mask  = mask;
        e->block = block;
        if (path)
            strncpy(e->path, path, IG_MAX_LKM_PATH - 1);
        write_unlock_irqrestore(&ig_policy_lock, flags);
        return 0;
    }
    write_unlock_irqrestore(&ig_policy_lock, flags);
    ...
    write_lock_irqsave(&ig_policy_lock, flags);
    hash_add(ig_policy_ht, &e->node, key);
    write_unlock_irqrestore(&ig_policy_lock, flags);
...
void inode_policy_clear(void)
{
    ...
    write_lock_irqsave(&ig_policy_lock, flags);
    hash_for_each_safe(ig_policy_ht, bkt, tmp, e, node) {
        hash_del(&e->node);
        kfree(e);
    }
    write_unlock_irqrestore(&ig_policy_lock, flags);
                        ...
                        uint32_t op, uint32_t *out_block)
{
    ...
    read_lock_irqsave(&ig_policy_lock, flags);
    e = find_entry(dev, ino);
    if (e && (e->mask & op)) {
        *out_block = e->block;
        found = 1;
    }
    read_unlock_irqrestore(&ig_policy_lock, flags);
    return found;
}
...
const char *inode_policy_path(uint64_t dev, uint64_t ino)
{
    ...
    struct ig_policy_entry *e = find_entry(dev, ino);
    return e ? e->path : "";
}
